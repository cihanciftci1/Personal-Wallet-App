...
export default function UpdateWallet() {
    ...
    function handleSubmit(event) {
        ...
        dispatch(updateWallet(wallet, walletId, navigate));
                    ...
                    <div className="col-md-8 m-auto">
                        ...
                        <form onSubmit={handleSubmit} action="dashboard.html">
                            <div className="form-group">
                                <input type="text" onChange={handleChange} name="name" value={wallet.name || ""} className={classnames("form-control form-control-lg", { "is-invalid": error.name })} placeholder="Account Name" />
                                {error.name && (<div className='invalid-feedback'>{error.name}</div>)}
                            </div>
                            <br />
                            <div className="form-group">
                                <input type="number" onChange={handleChange} name="accountNumber" value={wallet.accountNumber || ""} className={classnames("form-control form-control-lg", { "is-invalid": error.accountNumber })} placeholder="Account No" />
                                {error.accountNumber && (<div className='invalid-feedback'>{error.accountNumber}</div>)}
                            </div>
                            <br />
                            <div className="form-group">
                                <textarea onChange={handleChange} name="description" value={wallet.description || ""} className={classnames("form-control form-control-lg", { "is-invalid": error.description })} placeholder="Description"></textarea>
                                {error.description && (<div className='invalid-feedback'>{error.description}</div>)}
                            </div>
                            <br />
                            <div className="form-group">
                                <input type="number" onChange={handleChange} name="currentBalance" value={wallet.currentBalance || 0} className={classnames("form-control form-control-lg", { "is-invalid": error.currentBalance })} placeholder="Balance" />
